...
def function_3(): #reconstructing the original X string by using traces
    probability_list = []
    recovered_X = []
    ...
    for i in range(1,n+1):
        ...
        sum_known = int(probability_list[j-1]) - # don't know how to write the sigma sum?

        if sum_known >= 3/4*prob_i_j:
            recovered_X[i-1] = 1
        else:
            recovered_X[i-1] = 0
    return recovered_X
